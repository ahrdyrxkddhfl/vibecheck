...
파일 수집과 README 청킹에서 실제로 났던 버그들, 그리고 Java 지원에서 처음 확인한 규칙(Javadoc 범위, 오버로딩한 메서드의 id)을 고정해둔 테스트입니다.
...
개요, 면접 질문, 관계도 세 탭으로 보여줍니다. 면접 질문 탭에서 바로 답을 쓰고 채점받을 수 있습니다.
...
호출 관계는 파이썬 파일만 분석합니다. Java 파일을 열면 분석하지 않는다고 밝히고 함수 목록만 보여줍니다.
...
틀린 화살표를 긋는 대신 그리지 않습니다. 설계 결정 질문의 재료(코드에 남은 특이 지점)는
...
- [x] 웹 인터페이스 — 개요, 면접 질문과 답변 채점, 파일을 누르며 따라가는 호출 관계도
